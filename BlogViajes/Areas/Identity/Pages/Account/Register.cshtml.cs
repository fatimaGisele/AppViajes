...
namespace BlogViajes.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await _roleManager.RoleExistsAsync(Constant.Administrador))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(Constant.Administrador));
                        await _roleManager.CreateAsync(new IdentityRole(Constant.UsuarioSimple));
                    }

                    string rolSeleccionado = Request.Form["UsuarioRol"].ToString();

                    if (rolSeleccionado == Constant.Administrador) {
                        await _userManager.AddToRoleAsync(user, Constant.Administrador);
                    }
                    else
                    {
                        if(rolSeleccionado == Constant.UsuarioSimple)
                        {
                           await _userManager.AddToRoleAsync(user, Constant.UsuarioSimple);
                        }
                    }
